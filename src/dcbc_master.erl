...
-record(subp, {pid = none, status = none, slave_pid = none, ref = none}).
...
init([SolverArgs, FileNames]) ->
    ...
    Stubs = dict:from_list(lists:map(fun (Name) -> {Name, #subp{}} end, FileNames)),
...
handle_cast({solver_done, Name, Status, Val, Sol, _Log}, State0) ->
    io:format("~.3f Subproblem ~s status: ~p, result=~p~n", [seconds_elapsed(State0#state.start_ts), Name, Status, Val]),
...
submit_problem(Name, {State, [SlavePid | Tail] = Slaves}) ->
    ...
    case dcbc_slave:start_solver(SlavePid, Name,
        ...
        {ok, SolverPid} ->
            ...
            Subp = (dict:fetch(Name, State#state.stubs))#subp{
                     pid = SolverPid, slave_pid = SlavePid, ref = Ref, status = running},
            {State#state{stubs = dict:store(Name, Subp, State#state.stubs)}, Slaves};
        {error, _} ->
...
update_best(Name, Val, Sol, #state{best_val = BV1, best_sol = {BV2, _BS}} = State) ->
    case {is_better_than(Val, BV1, minimize), is_better_than(Val, BV2, minimize), Sol} of %% NOTE: minimization
        {true, true, Sol} when Sol =/= none ->
            io:format("~.3f ~s New best_val and best_sol: ~p~n", [seconds_elapsed(State#state.start_ts), Name, Val]),
            broadcast_best_val(Name, Val, State#state.stubs),
            State#state{best_val = Val, best_sol = {Val, Sol}};
        {true, _, Sol} ->
            io:format("~.3f ~s New best_val: ~p~n", [seconds_elapsed(State#state.start_ts), Name, Val]),
            broadcast_best_val(Name, Val, State#state.stubs),
            State#state{best_val = Val};
        {false, true, Sol} when Sol =/= none ->
            io:format("~.3f ~s New best_sol: ~p~n", [seconds_elapsed(State#state.start_ts), Name, Val]),
...
handle_info({'DOWN', Ref, process, _Pid, Reason}, State) ->
    ...
    io:format("Solver crashed for subproblem ~s: ~p~n", [Name, Reason]),
